...
class Template
{
	...
	public function __construct()
	{
        ...
        $this->_ci->load->config('application');
		$config = $this->_ci->config->item('template');
	...
	public function build_template_data()
	{
		...
		$template['page_title']		= empty($this->_pageTitle) ? $title : $this->_pageTitle;
    ...
    public function pageTitle()
    {
        ...
        if ($title_segments = func_get_args())
        {
            $this->_pageTitle = implode($this->_title_separator, $title_segments);
        }
	...
	public function set_theme($theme = null)
	{
		...
		foreach ($this->_theme_locations as $location)
		{
			...
			if (!file_exists($location.$this->_theme))
			{
                show_error('Unable to load the requested theme: '.$this->_theme);
            }
		}
	 ...
	 * @return	$this
	 */
	public function set_breadcrumb($name, $uri = '', $active = false, $reset = false, $override = false)
	{
		...
		if ($override) {
			$this->_override_breadcrumbs[] = array('name' => $name, 'uri' => $uri, 'active' => $active);
		} else {
			$this->_breadcrumbs[] = array('name' => $name, 'uri' => $uri, 'active' => $active);
		}
    ...
    {
        $content = self::_load_view('layouts/error_404', array(), false, self::_find_view_folder());
    ...
    public function show_404($message = '', $data = array(), $layout='error_404')
    {
        is_array($data) OR $data = (array) $data;
        ...
        $this->_ci->output->set_status_header(404, $message);
        $this->append_js('webpack::frontend-404.js', false, 'frontend-404');
        $this->append_css('webpack::frontend-404.css', false, 'frontend-404');
        $this->build('partials/error_404_body', array('message' => $message));
    }
}



function file_partial($file = '', $vars = array(), $ext = 'php')
{
    $CI = & get_instance();

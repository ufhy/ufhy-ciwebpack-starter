...
class Events
{
	...
	private static function _load_modules()
	{
		...
		//$_ci->load->model('addons/module_model');
		if ( ! $results = $_ci->enabledModules)
		{
			return false;
		}
		...
		foreach ($results as $row)
		{
			...
			if (!$details_class = self::_spawn_class($row['slug']))
			{
				continue;
			}
		}
	 ...
	 */
	private static function _spawn_class($slug)
	{
		$path = APPPATH;

		// Before we can install anything we need to know some details about the module
		$events_file = $path.'modules/'.$slug.'/Events_'.strtolower($slug).EXT;

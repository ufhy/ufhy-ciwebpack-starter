...
class Asset {
	...
	*/
	...
	public function __construct()
	{
		...
		$paths = $ci->config->item('assets.asset_paths')
            ? $ci->config->item('assets.asset_paths')
            : self::$asset_paths;
		...
		self::$symlinks = $ci->config->item('assets.asset_symlinks');

		self::$asset_url = $ci->config->item('assets.asset_url')
            ? $ci->config->item('assets.asset_url')
            : $ci->config->item('base_url');

		self::$default_folders = array(
			'css' => $ci->config->item('assets.asset_css_dir'),
			'js' => $ci->config->item('assets.asset_js_dir'),
			'img' => $ci->config->item('assets.asset_img_dir'),
		);

		self::$cache_path = $ci->config->item('assets.asset_cache_path');
		self::$default_options['min'] = $ci->config->item('assets.asset_min');
		self::$default_options['combine'] = $ci->config->item('assets.asset_combine');
		self::$deps_max_depth = $ci->config->item('assets.asset_deps_max_depth');

		$group_sets = $ci->config->item('assets.asset_groups');
		...
		self::$show_files = $ci->config->item('assets.asset_show_files');
		self::$show_files_inline = $ci->config->item('assets.asset_show_files_inline');
		self::$post_load_callback = $ci->config->item('assets.asset_post_load_callback');
		self::$filepath_callback = $ci->config->item('assets.asset_filepath_callback');
	}
    ...
    public static function getListJs($group = false, $inline_dep = null, $attr_dep = array())
    {
        // Do nοt force the user to remember that false is used for ommitted non-bool arguments
        if (!is_string($group)) {
            $group = false;

...
class MY_Model extends CI_Model
{
    ...
    public function where($field_or_array = NULL, $operator_or_value = NULL, $value = NULL, $with_or = FALSE, $with_not = FALSE, $custom_string = FALSE)
    {
        ...
        {
            $this->_database->{$where_or}(array($this->_database->dbprefix.$this->table.'.'.$this->primary_key => $field_or_array));
        }
        ...
        {
            $this->_database->{$where_or}(array($this->_database->dbprefix.$this->table.'.'.$field_or_array => $operator_or_value));
        }
        elseif(!isset($value) && isset($field_or_array) && isset($operator_or_value) && is_array($operator_or_value) && !is_array($field_or_array))
        {
            //echo $field_or_array;
            //exit;
            $this->_database->{$where_or.$not.'_in'}($this->_database->dbprefix.$this->table.'.'.$field_or_array, $operator_or_value);
        }
    ...
    public function _get_joined($requested)
    {
        ...
        if(!empty($requested['parameters']))
        {
            if(array_key_exists('fields',$requested['parameters']))
            {
                ...
                {
                    $sub_select[] = ((strpos($field,'.')===FALSE)
                            ? $this->_relationships[$requested['request']]['foreign_table'] . '.' . trim($field)
                            : trim($field)).' AS '.$requested['request'].'_'.trim($field);
                }
    ...
    public function get_all_count($where = NULL)
    {
                ...
                if(!empty($request['parameters']))
                {
                    if(array_key_exists('fields',$request['parameters']))
                    {
                        ...
                        else
                        {
                            ...
                            $fields = (!is_array($request['parameters']['fields'])) ? explode(',', $request['parameters']['fields']) : $request['parameters']['fields'];
                            foreach ($fields as $field)
                            {
                    ...
                    foreach($this->{$option} as $key => $relation)
                    {
                        ...
                        else
                        {
                            ...
                            else
                            {
                                ...
                                $model = $this->_parse_model_dir($foreign_model);
                                $foreign_model = $model['model_dir'].$model['foreign_model'];
                                $foreign_model_name = basename($foreign_model); //$model['foreign_model_name'];
    ...
    private function _where_trashed()
    {
        ...
        {
            case 'only' :
                $this->_database->where($this->_database->dbprefix.$this->table.'.'.$this->_deleted_at_field.' IS NOT NULL', NULL, FALSE);
                break;
            case 'without' :
                $this->_database->where($this->_database->dbprefix.$this->table.'.'.$this->_deleted_at_field.' IS NULL', NULL, FALSE);
                break;
            case 'with' :
                break;
        }
     ...
     * @return $this
     */
    public function fields($fields = NULL, $usingTable = TRUE)
    {
        if(isset($fields))
        {
            if($fields == '*count*')
            ...
            else
            {
                ...
                if (!empty($fields))
                {
                    foreach ($fields as &$field)
                    {
                        ...
                        {
                            $field = $usingTable ? $this->table . '.' . $field : $field;
                        }
                    }
                }
                $this->_select = $fields;
    ...
    {
    	$data['created_by'] = $this->the_auth->getUserLoginId();
    	return $data;
    }

    public function add_updater($data)
    {
	    $data['updated_by'] = $this->the_auth->getUserLoginId();
	    return $data;
    }

    public function add_deleted($data)
    {
        $data['deleted_by'] = $this->the_auth->getUserLoginId();
        return $data;
    }
    ...
    public function set_before_update($method)
    {
        $this->before_update[] = $method;
        return $this;
    }

    public function remove_before_update($method)
    {
        unset($this->before_update[$method]);
        return $this;
    }

    public function clear_before_update()
    {
        $this->before_update = [];
        return $this;
    }

    public function set_before_create($method)
    {
        $this->before_create[] = $method;
        return $this;
    }

    public function remove_before_create($method)
    {
        unset($this->before_create[$method]);
        return $this;
    }

    public function clear_before_create()
    {
        $this->before_create = [];
        return $this;
    }

    public function set_before_soft_delete($method)
    {
        $this->before_soft_delete[] = $method;
        return $this;
    }

    public function remove_before_soft_delete($method)
    {
        unset($this->before_soft_delete[$method]);
        return $this;
    }

    public function clear_before_soft_delete()
    {
